...
for (let i=0; i < wordsArr.length; i++) {
 ...
 wordIndex += wordsArr[i].length;

}     

return average = wordIndex/wordsArr.length;
...
let current = wordsUnique[i];
if (newArray.indexOf(current) < 0 ) newArray.push(current);
...
function doesWordExist(wordsFind, strings) {
...
for (i = 0; i < wordsFind.length; i++){
  ...
  if (wordsFind[i] == strings) return true;
  
  else if(wordsFind[i] != strings) return false;
